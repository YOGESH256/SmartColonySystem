...
const hi = async() => {
    ...
    var handleChange =  async(e) => {
      ...
      const lo =  await Promise.all([
        fetch(`http://open.mapquestapi.com/geocoding/v1/address?key=4kNrxkAjBDGNKdvOKlAhbAZfmlG7qOgM&postalCode=${pin}`),
        ]).then(async([aa]) => {
      ...
      return (<>
        ...
        <div className = "container classH" >
        ...
        <small id="emailHelp" className="form-text text-muted">Enter Your Pincode</small>
        </div>
